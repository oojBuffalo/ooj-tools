...
import { CONFIG_NAME, DEFAULT_CONFIG, relPosix } from "./config.js";
...
export function cmdInit(root, cfg, args) {
...
export function syncAll(root, cfg, write) {
    ...
    for (const d of scopeDirs(root, cfg)) {
        ...
        if (next !== old) {
            changed += 1;
            if (write)
                writeFileSync(dp, next);
        }
    }
    return changed;
}
export function cmdSync(root, cfg) {
    const n = syncAll(root, cfg, true);
    console.log(`dox: synced index in ${n} doc(s)`);
    return 0;
}
export function cmdCheck(root, cfg, args) {
    const doc = cfg.doc_name;
    const hard = [];
    const warn = [];
    ...
    for (const d of scopeDirs(root, cfg)) {
        ...
        if (cfg.enforce_sections !== false) {
            ...
            if (missing.length)
                warn.push(`SECTIONS  ${reld} missing: ${missing.join(", ")}`);
        }
    ...
    for (const w of warn)
        console.log(`dox: warn  ${w}`);
    for (const h of hard)
        console.log(`dox: ERROR ${h}`);
    if (hard.length) {
...
export function cmdContext(root, cfg, args) {
    const doc = cfg.doc_name;
    ...
    if (rel.startsWith(".."))
        rel = ""; // outside the root
    const parts = !rel || rel === "." ? [] : rel.split("/").filter(Boolean);
    const chain = [];
    let cur = root;
    if (existsSync(join(cur, doc)))
        chain.push(join(cur, doc));
